...
namespace Configurable_Injector {
	...
	extern "C" void* ProcessUtility( int Opcode, void *Parameters );
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->groupBox1->Size = System::Drawing::Size(250, 152);
			...
			this->label3->Location = System::Drawing::Point(6, 131);
			...
			this->label6->Text = L"Injection Mode: <NULL>";
			// 
			// Form1
			// 
...
#pragma endregion
	...
	private: System::Void Form1_Load(System::Object^  sender, System::EventArgs^  e) {
				 ...
				 if( *(bool *)ProcessUtility( OP_RequestDebugPrivledges, NULL ) == false )
				 {
					 Windows::Forms::MessageBox::Show(
						 "Warning: Unable to obtain debug privledges..", "WARNING", 
				 ...
				 String ^RawMode = GetXmlString( xmlDoc, "inj", 0 );

				 if( RawMode == nullptr )
				 {
					 m_InjectionMode = OP_InjectModuleFile;
				 }
				 else
				 {
					 if( RawMode->CompareTo( "MEMORY" ) == 0 )
					 {
						 m_InjectionMode = OP_InjectModuleFileToMemory;

						 this->label6->Text = "Injection Mode: MEMORY";
					 }
					 else
					 {
						m_InjectionMode = OP_InjectModuleFile; //default...

						this->label6->Text = "Injection Mode: FILE";
					 }
				 }
	...
	private: System::Void timer1_Tick(System::Object^  sender, System::EventArgs^  e) {
				 ...
				 for( int i = 0; i < pProcesses->Length; i++ )
				 {
					 ...
					 if( bMetConditions )
					 {
						 ...
						 if( this->m_InjectionMode == OP_InjectModuleFileToMemory )
						 {
							 FParams_InjectModuleFileToMemory Params;

							 Params.File		= wcLibraryString;
							 Params.hProcess	= ( HANDLE )iHandle;

							 if( *(bool *)ProcessUtility( OP_InjectModuleFileToMemory, &Params ) == true )
							 {
						 ...
						 {
							 FParams_InjectModuleFile Params;

							 Params.File		= wcLibraryString;
							 Params.hProcess	= ( HANDLE )iHandle;

							 if( *(bool *)ProcessUtility( OP_InjectModuleFile, &Params ) == true )
							 {
								 timer1->Enabled = false;
